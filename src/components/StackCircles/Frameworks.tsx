import { SiSpringboot, SiNextdotjs, SiElectron, SiAngular, SiDotnet, SiVitest } from "react-icons/si";

function Frameworks() {
  return (
    <>
      <div className="absolute dark:text-white text-[2rem] flex gap-4 flex-col w-[400px] h-[400px] animate-rotate-reverse group-hover:pause-reverse border rounded-full group hover:border-sky-600 dark:hover:border-sky-400 hover:pause peer/frameworks border-slate-900 dark:border-white">
        ...
        <div className="flex justify-between items-center h-full w-[60%] place-self-center">
          ...
          <span className="transition-all hover:text-yellow-400 relative animate-rotate group-hover:pause">
            <SiVitest className="peer hover:scale-125" />
            <span className="absolute left-10 top-0 w-0 whitespace-nowrap peer-hover:w-48 transition-all duration-500 text-2xl font-bold overflow-hidden peer-hover:animate-slideright">
              Vitest
            </span>
          </span>
      ...
      <span className="text-3xl font-bold tracking-[1rem] absolute top-[50px] opacity-0 peer-hover/frameworks:opacity-100">Frameworks</span>
    </>
  );
}
